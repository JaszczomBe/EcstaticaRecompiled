...
7. Recovered the later FAN tail path through ordinal-4 termination, `FUN_00447638` `49650` terrain/path entries, `1200` segment records, tail-node allocation, post-FAN invalid actor-head cleanup, hosted literal path validation, and stable `Files/ECSTATIC` archive opening. Debug now reaches `FUN_00447d94`; the current crash frontier is `FUN_00447d94 -> FUN_0043cac0 -> FUN_0041ad54` with bad fill rectangle state.
