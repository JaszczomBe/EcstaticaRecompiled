...
The active frontier is no longer `PlayScene "horse"` resolution. Original `E2WIN95.EXE` disassembly proves script opcodes `0x07` and `0x0c` should pass the loaded scene record to `FUN_004523f8`; the generated reconstruction now mirrors that register flow. `E2R_SCRIPT_DIAG=1` proves token `0x061c` resolves to `horse`, loads from flat-FANT offset `607214`, installs slot `0xac2594`, and gains active flag `0x02` after activation. The missing piece is actor/current ownership after activation: `_DAT_0073cc3c` and `DAT_0047a470` remain `0x0`, `E2R_RUNTIME_DIAG=1` frame-loop traces show `current=0x0 actors=0x0 links=0x0`, actor `0` enters `FUN_00451f5c` with `table=0x0`, and the first post-startup dumps still show the logo surface instead of E2WIN95's horseback/credits intro.
