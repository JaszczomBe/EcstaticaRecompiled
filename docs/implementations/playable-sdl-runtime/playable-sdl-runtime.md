...
3. Recovered `Start_sc` completion into gameplay: scene-child actor live-vector initialization now preserves Joe at `12800,256,-3584`, signed scene-height lookup treats descriptor byte `0xff` as passable, `FUN_0044c224` selects scene `87`, and a delayed `Up` probe latches movement in scene `0x67d0ac`.
