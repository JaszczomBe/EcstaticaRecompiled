...
Task 02 completed the compact arrow movement matrix and corrected the host translation that had made physical arrows look like numpad keys. Carry `Space` as a narrower action-dispatch/completion frontier: it is delivered and latched, but the intro action still completes naturally and dispatches opcode `0x07` into scene `7`. Task 03's freshest menu sub-slice should first manually confirm the real SDL backdrop-stack behavior: one Escape hides the active main menu, Settings/Load close back to the main menu, and Settings row/OK clicks work. Then continue Load title/slot/OK and requester visual fidelity before returning to camera/action/modifier-state proof.
