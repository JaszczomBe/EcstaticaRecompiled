...
4. `Start_sc` now runs to completion and preserves the pedestal scene after the handoff. The scene lookup still proposes descriptor `48771`/scene `87`, but the repaired handoff marks the exact `Start_sc` action completion and keeps current scene slot `175` instead. The latest long probe exits without the user-reported crash, keeps `_DAT_0073cc3c=0x67da4c`, opens `hires\0175.raw`, and dumps nonblank surface 3 hash `321e33d6` with Joe visible. Final gameplay-control readiness in this scene remains open. Full horseback animation/procession fidelity remains separate from this fastest-route proof.
...
Task 02 completed the compact arrow movement matrix and corrected the host translation that had made physical arrows look like numpad keys. The fastest original intro route now reaches the visible pedestal scene without the post-subintro crash: first-intro `Space` skips into `Start_sc`, Space during the pedestal/lightning sub-intro is consumed and ignored, the sub-intro completes, and the one-shot handoff guard preserves scene `175` when the scene selector proposes scene `87`. Task 03's freshest direction is gameplay-control readiness in scene `175`, then camera/action/modifier classification and requester visual polish; full first-intro animation fidelity remains a separate follow-up.
...
2. Added a one-shot `Start_sc` action-completion handoff guard in generated reconstruction code. The long dummy-SDL route now exits with scene pointer `0x67da4c`, `hires\0175.raw`, surface 3 hash `321e33d6`, and Joe visible in the pedestal scene.
