...
9. Original-game menu testing showed the intro menu is mouse-only. The SDL backend now maps host mouse coordinates through the aspect-fit presentation rectangle, the Win32 shim preserves cursor position, and the reconstructed WndProc feeds requester mouse state. Bounded intro-menu click probes now route Settings to requester id `0x31`/state `2` and Load to requester id `0x29`/state `4` without crashing. The raw surface dumps from these probes are nonblank but not yet useful as visual submenu screenshots, so treat the log state as the current proof.
