...
3. Notes: Final task for Step 3; intro gameplay-entry recovery is now proven, with requester visual/slot behavior still pending but lower priority unless it blocks probes. Resume the original camera/action/modifier classification from the completed arrow movement matrix and keep full first-intro animation fidelity split from this gameplay-route proof. Classify `Ctrl`/`Shift`/`Left Alt`/`Right Alt`, `Return`, `S`, `I`, and any version-specific `L` quickload report before wiring unproven gameplay behavior.
...
4. Recovered the full fastest gameplay path: `--inject-key-sequence-surfaces /tmp/e2-startsc-gameplay-up space,up 4 40000 65` skips the first intro, waits through `Start_sc`, posts `Up` in scene `0x67d0ac`, and exits with `move=[1,0,0,0,0,0,0,0,0]`.
